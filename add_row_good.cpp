...
int main() {
    ...
    for (auto &[m, c] : m_count) {
        ...
        if (!base_could_become_gog(m1, m2, m3))
            continue;
    ...
    std::cout << cnt_dif << " bugless Magog trapezoids, " << cnt_total << " total bugless Magog trapezoids\n";
    ...
    for (auto &[g, c] : g_count) {
        ...
        if (!base_could_become_magog(g1, g2, g3)) 
            continue;
        cnt_dif++;
        cnt_total += c;
    }

    std::cout << cnt_dif << " bugless Gog trapezoids, " << cnt_total << " total bugless Gog trapezoids\n";
    return 0;
